...
class Action_embedding:
    ...
    use_action_embedding = True if Configure.get(
        'Embedding', 'use_action_embedding').lower() == "true" else False
    action_dim = int(Configure.get('Embedding', 'action_vector_dim'))
    embedding_model = "SBERT"
    embedding_model_name = Configure.get(
        "Embedding", f"{embedding_model}_model")

    # manhattan angular  euclidean
    nn_metric = "angular"
    use_dim_reduction = True if Configure.get(
        'Embedding', 'use_dim_reduction').lower() == "true" else False
    normalization = True
    reduction_action_dim=int(Configure.get('Embedding', 'reduction_action_dim'))
    def __init__(self,
                 actions: list,
                 action_path: Path,
        ...
        if normalization:
            self.action_vector_checkout = self.action_path / \
    ...
    def build_annoy(self):
        ...
        index.build(
            num_trees,
            n_jobs=-1)  # n_trees 表示树的棵数，n_jobs 表示线程个数，n_jobs=-1 表示使用所有的 CPU 核；
        return index
    ...
    def build_action_embedding(self):
        if self.action_dim == 1:
            self.vector_space = self.init_uniform_space(low=[-1],
                                                        high=[1],
                                                        points=len(
                                                            self.action_data))
            return
    ...
    def search_point(self, point, k_nearest_neighbors):  # point:[dim,],numpy
        p_in = point
        if not isinstance(point, np.ndarray):
            p_in = np.array([p_in]).astype(np.float32)
        # p_in = self.import_point(point)
        search_res, _ = self.flann.nn_index(p_in, k_nearest_neighbors)
        knns = self.vector_space[search_res]
        p_out = []
        for p in knns:
        ...
        if k_nearest_neighbors == 1:
            p_out = [p_out]
        # p_out是实际环境可执行的动作
        return knns, np.array(p_out), search_res.tolist()[0]
        #      [k,batch,dim]
    ...
    def init_SBERT_action_embedding(self):
        ...
        return sentence_embeddings(sentences=action_descriptions,normalization=self.normalization)
    ...
    def init_uniform_space(self, low, high, points):
        ...
        axis = []
        for i in range(dims):
            axis.append(list(np.linspace(low[i], high[i],
                                         points_in_each_axis)))
    ...
    # def get_BERT_action_vector(self, action: str):
    #     des = self.get_action_description(action)
    #     vector = get_vector(text=des, dim=self.action_dim)
    #     return vector.detach().numpy().flatten().astype(np.float32)

    def get_action_index(self, wolp_action):
        # index=0
        # if not isinstance(wolp_action, np.ndarray):
        #     wolp_action = np.array([wolp_action]).astype(np.float32)
        # index=np.where(self.vector_space==wolp_action[0][0])
        search_res, _ = self.flann.nn_index(wolp_action[0], 1)
        return search_res.tolist()[0]
    ...
    def LSA_dim_reduction(self, dim=100):
        ...
        svd = make_pipeline(TruncatedSVD(n_components=dim),
...
class Action:
    vul_hub_path = Path(os.path.join(
        curr_path, Configure.get("common", "Vul_hub")))
    with open(vul_hub_path/"actions.json", 'r') as f:  # *********
        vulhub = json.loads(f.read())
    Vul_set = vulhub["vuls"]

    Vul_name_set = [v['name'] for v in Vul_set]
    All_EXP = [
        Action_Class(
            id=int(v['id']),
            name=v["name"],
    ...
    PORT_SCAN = Action_Class(id=-1, name="Port Scan", act_cost=0, success_reward=0,type="Scan")
    OS_SCAN = Action_Class(id=-2, name="OS Detect",
                           success_reward=100, act_cost=3, type="Scan")
    ...
    if Action_embedding.use_action_embedding:
        action_embedding = Action_embedding(
            actions=legal_actions_name, action_path=vul_hub_path)
    # types of errors
    # 1 重复动作执行
    ...
    def action_constraint(self,  action):
        ...
        if action == self.OS_SCAN or action in self.All_EXP:
            if not (self.PORT_SCAN.id in self.history_set
                    or self.PORT_SERVICE_SCAN.id in self.history_set):
                return self.action_dependence
        if action == self.WEB_SCAN:
            if not (self.SERVICE_SCAN.id in self.history_set
                    or self.PORT_SERVICE_SCAN.id in self.history_set):
                return self.action_dependence
